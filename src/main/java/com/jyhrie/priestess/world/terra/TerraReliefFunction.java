 ...
 *
 * <p>The counterpart to {@link TerraElevationFunction}, and independent of it so that a high
 * plateau and a broken lowland are both paintable.
 *
 * <p>Output is the map's [0,1] grey scaled to a density amplitude, where
 * {@code surfaceY = 128 + 128 * terrainHeight}, so a painted grey reads directly as a
 * number of blocks:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>That is the height of a spur, not its spacing ({@code rangeScale} in
 * {@code ModNoiseSettings}). Treat it as a centre rather than a cap: {@code reliefVariation}
 * multiplies it by 0.72–1.25 along a range, and it is damped to zero at the waterline.
 *
