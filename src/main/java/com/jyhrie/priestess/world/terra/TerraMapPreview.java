 ...
 * </pre>
 *
 * <p>The source PNGs are not the world: a two-scale domain warp and a bilinear filter sit
 * between them and the ground, and a warp strong enough to look organic is also strong
 * enough to tear a small nation apart. This samples through the same code path the chunk
 * generator uses, so what you see is what generates.
 *
 * <p>Left panel: region colours, shaded by terrain slot. Right panel: terrain slots alone.
 *
 * <p>It runs inside datagen rather than as a standalone {@code main} because
 * {@link TerraRegion} holds {@code ResourceKey}s, so touching one initialises
 * {@code Registries}, which throws unless the game has bootstrapped — and under Forge that
 * cannot happen outside the mod launcher.
 */
public final class TerraMapPreview {
    ...
    public static String render(File file) throws Exception {
        ...
        // Derived from world size so the preview stays the same size on screen whatever
        // scale the world is built at.
        ...
        // Following the origin shift is what makes the printed coordinates the ones players
        // actually stand on.
        ...
        // A region with no pixels is almost always a mistake: a colour in TerraRegion that
        // is not in regions.png.
        ...
        // A representative coordinate per region, to paste into /tp. Snapped to the nearest
        // sample actually inside the region, because a centroid alone lands outside anything
