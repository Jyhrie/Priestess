...
/**
 * Draws {@link Awaken} from its Blockbench model.
 *
 ...
 * below, so {@code new ResourceLocation(MOD_ID, "awaken")} means exactly:
 * <ul>
 *   <li>{@code assets/priestess/geo/entity/awaken.geo.json}</li>
 *   <li>{@code assets/priestess/textures/entity/awaken.png}</li>
 *   <li>{@code assets/priestess/animations/entity/awaken.animation.json}</li>
 * </ul>
 * The last of those does not exist yet and does not need to: GeckoLib resolves it lazily,
 * only when a controller asks for a clip by name, and {@code Awaken} registers none.
 *
 ...
 */
public class AwakenRenderer extends GeoEntityRenderer<Awaken> {

    /**
     * Draw scale. Must be kept in step with the hitbox on {@code ModEntities.AWAKEN}, which
    ...
    public AwakenRenderer(EntityRendererProvider.Context context) {
        super(context, new DefaultedEntityGeoModel<>(new ResourceLocation(Priestess.MOD_ID, "awaken")));
