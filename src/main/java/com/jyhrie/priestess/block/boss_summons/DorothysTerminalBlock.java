 ...
 *
 * <p>Fussy about space, and has to be: "Awaken" cannot walk, fall or be shoved out of a bad
 * spawn, so a summon into a corridor leaves a boss permanently inside a wall.
 * {@link #clearanceFor} checks a box a block wider than the hitbox on each side, which is the
 * difference between "it fits" and "you can fight around it".
 *
 * <p>That is an 8.75-block span against the {@code dorothys_vision} pit's nine blocks of clear
 * floor — expect this altar to refuse most rooms that are not purpose-built.
 */
